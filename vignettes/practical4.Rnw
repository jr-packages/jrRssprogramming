...
Before starting, we first install \cc{drat}\sidenote{If you have already installed \cc{drat} you can skip this step.}

<<eval=FALSE, tidy=FALSE>>=
install.packages("drat")
@

\noindent Then install the course R package via

<<eval=FALSE, tidy=FALSE>>=
drat::addRepo("rcourses")
install.packages("jrRssprogramming")
@
